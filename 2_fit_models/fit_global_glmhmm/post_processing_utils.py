...
def get_train_test_dta(inpt, y, session, session_fold_lookup_table,
                       fold):
    '''
    ...
    test_inpt, test_y, this_test_session = inpt[idx_test, :], y[idx_test,:], \
                                                          session[idx_test]
    train_inpt, train_y, this_train_session = inpt[idx_train,:], y[idx_train,:], \
                                                          session[idx_train]
    return test_inpt, test_y, this_test_session, train_inpt, \
           train_y, this_train_session
...
def prepare_data_for_cv(inpt, y, session, session_fold_lookup_table, fold):
    ...
    train_inpt, train_y, this_train_session = \
        get_train_test_dta(
            inpt, y, session, session_fold_lookup_table,
            fold)
    M = train_inpt.shape[1]
    n_test = len(test_y)
    n_train = len(train_y)
    return test_inpt, test_y, this_test_session, \
           train_inpt, train_y, this_train_session, \
           M, n_test, n_train
...
def return_glmhmm_nll(inpt, y, session, session_fold_lookup_table, fold, K, D,
    ...
    test_inputs, test_datas = \
        partition_data_by_session(
            test_inpt, test_y,
            this_test_session)
    train_inputs, train_datas = \
        partition_data_by_session(
            train_inpt, train_y,
            this_train_session)
    ...
    file_ordering_by_train = calculate_glm_hmm_test_loglikelihood(
        dir_to_check, test_datas, test_inputs, K, D,
        M, C)
    train_ll_vals_across_iters, _, _ = calculate_glm_hmm_test_loglikelihood(
        dir_to_check, train_datas, train_inputs, K,
        D, M, C)
...
def calculate_state_permutation(hmm_params):
    ...
    '''
    Edited version (Cecelia): no matter K=?, oredr by engagement
    '''
...
def get_marginal_posterior(inputs, datas, hmm_params, K, permutation):
